...
const mockGetPrimary  = vi.mocked(getLidarrPrimaryRoot);
...
function seedDb(db: Database.Database, opts: {
  ...
  const listId = (db.prepare(
    `INSERT INTO lists (name, root_folder_path, quality_profile_id, metadata_profile_id) VALUES ('TestList', ?, 1, 1) RETURNING id`
...
function getSyncStatus(db: Database.Database, itemId: number) {
  return (db.prepare('SELECT sync_status, sync_error, lidarr_artist_id FROM list_items WHERE id = ?').get(itemId) as
    { sync_status: string; sync_error: string | null; lidarr_artist_id: number | null });
}

const PRIMARY_ROOT = '/root/primary';
...
describe('Scenario A — artist type', () => {
  ...
  beforeEach(() => {
    ...
    mockGetPrimary.mockResolvedValue(PRIMARY_ROOT);
  });

  it('adds a new artist to Lidarr and marks synced when in primary root', async () => {
    const { itemId } = seedDb(db, { itemType: 'artist', itemMbid: 'artist-mbid-1', listRoot: PRIMARY_ROOT });
    ...
    expect(mockAddArtist).toHaveBeenCalledWith(expect.objectContaining({
      foreignArtistId: 'artist-mbid-1',
      rootFolderPath: PRIMARY_ROOT,
      addOptions: { monitor: 'all', searchForMissingAlbums: true },
    }));
    expect(mockRunCommand).toHaveBeenCalledWith('ArtistSearch', { artistId: 42 });

    const { sync_status, lidarr_artist_id } = getSyncStatus(db, itemId);
    expect(sync_status).toBe('synced');
    expect(lidarr_artist_id).toBe(42);
  });

  it('marks synced when artist already exists and list is in primary root', async () => {
    const { itemId } = seedDb(db, { itemType: 'artist', itemMbid: 'artist-mbid-1', listRoot: PRIMARY_ROOT });
    mockGetArtist.mockResolvedValue(lidarrArtist({ id: 55 }));

    await orchestrate(itemId);

    expect(mockAddArtist).not.toHaveBeenCalled();
    // No ArtistSearch for existing artists unless we specifically want to rescan, but currently we just update metadata
    const { sync_status, lidarr_artist_id } = getSyncStatus(db, itemId);
    expect(sync_status).toBe('synced');
    expect(lidarr_artist_id).toBe(55);
  });

  it('starts mirror backfill when artist is added but list is in secondary root', async () => {
    const { itemId } = seedDb(db, { itemType: 'artist', itemMbid: 'artist-mbid-1', listRoot: '/root/alt' });
    mockGetArtist.mockResolvedValue(lidarrArtist({ id: 55 }));

    await orchestrate(itemId);

    expect(mockAddArtist).not.toHaveBeenCalled();
    expect(mockBackfill).toHaveBeenCalledWith(55, itemId, PRIMARY_ROOT, '/root/alt', { type: 'artist' });
...
describe('Scenario B — track type', () => {
  ...
  it('adds artist (monitor=none) then monitors the track when artist not in Lidarr', async () => {
    const { itemId } = seedDb(db, { itemType: 'track', itemMbid: TRACK_MBID, artistMbid: ARTIST_MBID, listRoot: PRIMARY_ROOT });
    mockGetArtist.mockResolvedValue(null);
    mockAddArtist.mockResolvedValue(lidarrArtist({ id: 42 }));
  ...
  it('starts backfill when list is in secondary root', async () => {
    const { itemId } = seedDb(db, { itemType: 'track', itemMbid: TRACK_MBID, artistMbid: ARTIST_MBID, listRoot: '/root/alt' });
    mockGetArtist.mockResolvedValue(lidarrArtist({ id: 42 }));
    mockGetTracks.mockResolvedValue(mockTrackList);

    await orchestrate(itemId);

    expect(mockBackfill).toHaveBeenCalledWith(42, itemId, PRIMARY_ROOT, '/root/alt', { type: 'track', lidarrTrackId: 10, lidarrAlbumId: 5 });
    const { sync_status } = getSyncStatus(db, itemId);
    expect(sync_status).toBe('mirror_pending');
  });
});
...
describe('Scenario C — album type', () => {
  ...
  it('monitors the album when list is in primary root', async () => {
    const { itemId } = seedDb(db, { itemType: 'album', itemMbid: ALBUM_MBID, artistMbid: ARTIST_MBID, listRoot: PRIMARY_ROOT });
    mockGetArtist.mockResolvedValue(lidarrArtist({ id: 42 }));
  ...
  it('starts backfill when list is in secondary root', async () => {
    const { itemId } = seedDb(db, { itemType: 'album', itemMbid: ALBUM_MBID, artistMbid: ARTIST_MBID, listRoot: '/root/alt' });
    mockGetArtist.mockResolvedValue(lidarrArtist({ id: 42 }));
    mockGetAlbums.mockResolvedValue(mockAlbumList);

    await orchestrate(itemId);

    expect(mockBackfill).toHaveBeenCalledWith(42, itemId, PRIMARY_ROOT, '/root/alt', { type: 'album', lidarrAlbumId: 20 });
    const { sync_status } = getSyncStatus(db, itemId);
    expect(sync_status).toBe('mirror_pending');
  });
});
